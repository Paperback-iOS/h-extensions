...
export const parseMangaDetails = ($: CheerioStatic, mangaId: string): Manga => {
	...
	const altTitleArray: string[] | null = decodeHTMLEntity($(".entry-title", ".post").text()).replace(/(\[.+?\])/g, "").match(/(\(.+?\))/g);
	...
	switch (rawStatus) {
		...
		case "Ongoing":
			...
			break;
		case "Licensed":
			// Not supported by the app at the moment so falling back to MangaStatus.ONGOING
			status = MangaStatus.ONGOING;
			break;
		case "Dropped":
			// Not supported by the app at the moment so falling back to MangaStatus.ONGOING
	...
	for (const element of $("div > .jp-relatedposts-post-a").toArray()) {
		...
		if (mangaLink !== null && mangaLink.length > 0) {
			let mangaId = mangaLink.split("/").reverse()[1] ?? "";
			relatedIds.push(mangaId);
		}
	}
