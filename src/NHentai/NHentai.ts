...
export class NHentai extends Source {
	...
	async searchRequest(query: SearchRequest, metadata: RequestMetadata): Promise<PagedResults> {
		...
		if (query.title?.startsWith('https') || query.title?.startsWith('nhentai.net'))
		...
		const request = createRequestObject({
			url: QUERY(encodeURI(title), metadata.sort ? metadata.sort : '', metadata.nextPage), // If in the future sort becomes a thing.
			method: 'GET',
			headers: {
				'accept-encoding': 'application/json'
			}
